...
//                    $name = str_replace(" ", "-", $name);
                ...
                if ($model->validate()) {
                    ...
                    if (!is_dir($path)) {
                        ...
                        mkdir($path . '/optimized/', 0777, true);

                        chmod($path, 0777);
                        chmod($path . '/thumbnails/', 0777);
                        chmod($path . '/optimized/', 0777);
                    }

                    $newName->saveAs($path . $model->name);
                    $modelimage->listing_id = $_GET['listing_id'];
                    $modelimage->url =$model->name;
                    $modelimage->directory = $this->_subfolder;
                    ...
                    $thumbnail = new Image($path . $model->name);
                    $thumbnail->resize(180, 180, Image::AUTO); //

                    $optimized = new Image($path . $model->name);
                    $optimized->resize(600, 400, Image::AUTO);

                    $thumbnail->save(Yii::getPathOfAlias('media').'/galleries/' . $this->_subfolder . '/thumbnails/' . $model->name);
                    $optimized->save(Yii::getPathOfAlias('media').'/galleries/' . $this->_subfolder . '/optimized/' . $model->name);
                    ...
                    echo json_encode(array(array(
                            ...
                            "thumbnail_url" => $publicPath .'thumbnails/' . $model->name,
