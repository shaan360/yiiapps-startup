...
/**
 * AweController is the base class for the generated controllers.
 *
 * @author Ricardo Obregón <ricardo@obregon.co>
...
abstract class AweController extends Controller
{
     ...
     * @return AweActiveRecord The loaded model.
    ...
    public function loadModel($key, $modelClass)
    {
        ...
        if (is_array($key)) {
            ...
            if (key($key) === 0) {
                ...
                if (!is_array($tablePk)) {
                    throw new CHttpException(400, Yii::t('AweCrud.messages', 'Your request is invalid.'));
                }
        ...
        if ($model === null) {
            throw new CHttpException(404, Yii::t('AweCrud.messages', 'The requested page does not exist.'));
        }
    ...
    {
        if (isset($_REQUEST['ajax']) or isset($_REQUEST['noUI'])) {
            $output = $this->renderPartial($view, $data, $return);
    ...
    public function endResponseWithoutWebLogRoute()
    {
    ...
    protected function getRelatedData($form, $relations, $uncheckValue = '')
    {
        ...
        foreach ($relations as $relationName => $relationData) {
            if (isset($form[$relationName]) && (($relationData[0] == AweActiveRecord::HAS_MANY) || ($relationData[0] == AweActiveRecord::MANY_MANY))) {
