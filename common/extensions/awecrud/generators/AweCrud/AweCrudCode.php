...
Yii::import('common.extensions.AweCrud.helpers.*');
...
class AweCrudCode extends CrudCode
{
    ...
    public $useActiveRecordRelationBehavior = false;
    ...
    public function rules()
    {
        return array_merge(
            parent::rules(),
            array(
                array('authtype, validation', 'required'),
            )
        );
    }
    ...
    public function generateActiveField($modelClass, $column)
    {
        if ($column->isForeignKey) {
            ...
            if ($this->useActiveRecordRelationBehavior) {
    ...
    public function getNMField($relation, $relatedModelClass, $modelClass)
    {
        ...
        $str = "<label for=\"$relatedModelClass\"><?= Yii::t('app', '$friendlyName'); ?></label>\n";
        $str .= "<?= CHtml::checkBoxList('{$modelClass}[{$relatedModelClass}]', array_map('AweHtml::getPrimaryKey', \$model->{$relatedModelClass}),
