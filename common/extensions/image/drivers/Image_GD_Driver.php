...
class Image_GD_Driver extends Image_Driver {
	...
	public function colorize($properties)
	{
		...
		imagecopy($img, $this->tmp_image, 0, 0, 0, 0, $width, $height);
		
		// Execute the colorize
    	if($status = imagefilter($img, IMG_FILTER_COLORIZE, $r, $g, $b))
    	{    	
    		// is there an overlay?
    		if($overlay)
    		{
    			...
    			imagecopy($img, $over, 0, 0, 0, 0, $width, $height);    			
    		}
